...
public class TransactionLogDB {
    ...
    public static List<TransactionLog> getFilteredTransactionLogs(String selectedItem){
        List<TransactionLog> logs = new ArrayList<>();
        try (PreparedStatement statement = DBConnector.connect.prepareStatement(
                "SELECT * FROM transaction_log WHERE item = ?")) {
            statement.setString(1, String.valueOf(selectedItem));
